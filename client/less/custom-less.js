...
//
// Modified by Hansi Raber (2012), 
// all changes are marked with ---PATCH
...
function loadStyles() {
    ...
    for (var i = 0; i < styles.length; i++) {
        ...
        if (styles[i].type.match(typePattern) || styles[i].hasAttribute( "lessText" )) {
            ...
            new(less.Parser)().parse( lessText, function (e, tree) {
                var css = tree.toCSS();
                var style = styles[i];
                style.type = 'text/css';
                if (style.styleSheet) {
